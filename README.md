...
Combinations: When you just have to choose some objects from a larger set and the order is of no significance, then the rule of counting that you use is called combination. For example, if you need to find the number of ways to choose three vowels from the given list of five, or choose four bowlers from the given roster of seven – all these methods use combinations. If you want to choose 'r' objects from a larger set of 'n' objects, then the number of ways in which you can do that is given by the following formula:
## nCr = n!/(n-r)!r!
